...
public:
	...
	DWORD GetWebFile(LPCTSTR pszAppName, 
					 LPCTSTR lpstrServer,
					 int nPort, 
					 CString& strFile, CString& strOutFile);
   DWORD GetWebFile2(LPCTSTR pstrAgent, LPCTSTR lpstrServer, int nPort, CString& strFile, 
	  CString& strOutFile, int iSize, CWnd* pWnd);

   DWORD GetWebFile(LPCSTR lpcstrFile, CString& strOutFile);
