...
int CPPToolTip::FindTool(CWnd * pWnd, LPCRECT lpRect /* = NULL */)
{
    ...
    for (int i = 0; i < nSize; i++)
    {
        ...
        if (hWnd == pToolInfo.hWnd) 
			if ((NULL == lpRect) || 
			   (lpRect->left == pToolInfo.rectBounds.left && 
			    lpRect->right == pToolInfo.rectBounds.right && 
				lpRect->top == pToolInfo.rectBounds.top && 
				lpRect->bottom == pToolInfo.rectBounds.bottom))
			return i;
    }
	return PPTOOLTIP_TOOL_NOEXIST;
}
